...
export class OraoVrfTestHelper {
  static readonly VRF_PROGRAM_ID = new PublicKey(
  ...
  private async init(): Promise<void> {
    const programInfo = await this.connection.getAccountInfo(
      OraoVrfTestHelper.VRF_PROGRAM_ID
    );
    if (!programInfo) {
      throw new Error(
        "Orao VRF program is not loaded into your localnet, please check it..."
      );
    }
  ...
  async mockFulfillment(seed: Buffer): Promise<void> {
    let signature = nacl.sign.detached(
      seed,
      this.fulfillmentAuthority.secretKey
    );
